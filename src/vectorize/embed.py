...
from vectorize.config import EmbedderConfig, ModelConfig
...
class Embedder:
    ...
    def __init__(
        ...
        if self.enable_local_cache:
            self.cache_limit_size = config.cache_limit_size
    ...
    def _load_cached_embeddings(
        ...
        for h in hashes:
            cache_path = os.path.join(self.local_cache_dir, f"{h}.npy")
    ...
    def embed_with_cache(
        ...
        """
        Gera embeddings de textos, de maneira eficiente, utilizando divisão em lotes e cache local.
        Busca e reutiliza embeddings existentes no cache local antes de computar novos embeddings.
        ...
        for i in range(
            ...
            if self.enable_local_cache:
                self._cache_embeddings(batch_hashes, batch_embeddings)
