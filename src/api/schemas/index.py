...
class IndexRequest(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(..., description="Metadados adicionais associados ao documento")


class IndexResponse(BaseModel):
    """Schema para a resposta de indexação de documento."""
    doc_id: str = Field(..., description="ID do documento indexado")
    status:  Literal["success", "failure"] = Field(..., description="Status da operação de indexação")
    message: Optional[str] = Field(None, description="Mensagem adicional sobre o resultado da indexação")
